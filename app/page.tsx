...
import { accounts as initialAccounts, monthlyData as initialMonthlyData, calculateNetWorth } from "@/lib/data"
...
export default function WealthTracker() {
  ...
  const handleDeleteAccount = (accountId: number) => {
    if (confirm("Are you sure you want to delete this account?")) {
      ...
      Object.keys(updatedMonthlyData).forEach((month) => {
        updatedMonthlyData[month] = updatedMonthlyData[month].filter((entry) => entry.accountId !== accountId)
      })
      setMonthlyData(updatedMonthlyData)
    }
  }

  const handleUpdateMonthlyEntry = (accountId: number, month: string, updatedEntry: Partial<MonthlyEntry>) => {
    const updatedMonthlyData = { ...monthlyData }
    ...
    const entryIndex = updatedMonthlyData[month].findIndex((entry) => entry.accountId === accountId)
  ...
  const handleAddNewMonth = (accountId: number, month: string, entry: MonthlyEntry) => {
    // Check if month already exists for this account
    if (monthlyData[month]?.some((e) => e.accountId === accountId)) {
      alert("An entry for this month already exists. Please edit the existing entry instead.")
      return
    }
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto py-4 px-4 max-w-7xl">
        ...
        <div className="text-center mb-6 sm:mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold mb-2">💰 Wealth Tracker</h1>
          <div className="text-3xl sm:text-4xl lg:text-5xl font-bold text-green-600">£{netWorth.toLocaleString()}</div>
        </div>
