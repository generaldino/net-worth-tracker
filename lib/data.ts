...
export function getPreviousBalance(accountId: number, month: string): number {
  ...
  const previousEntry = monthlyData[previousMonth]?.find((e) => e.accountId === accountId)
...
export function calculateNetChange(accountId: number, month: string, endingBalance: number): number {
  const previousBalance = getPreviousBalance(accountId, month)
  return endingBalance - previousBalance
}

export function calculateAccountGrowth(accountId: number, month: string, entry: MonthlyEntry): number {
...
export function calculateNetWorth(): number {
  return accounts.reduce((sum, account) => {
    return sum + getCurrentValue(account.id)
  }, 0)
}
...
export function getFilteredMonths(timePeriod: TimePeriod): string[] {
  ...
  switch (timePeriod) {
    case "YTD":
      return allMonths.filter((month) => month.startsWith(currentYear.toString()))
    case "1Y":
...
export function getNetWorthOverTime(timePeriod: TimePeriod = "all"): Array<{ month: string; netWorth: number }> {
  const months = getFilteredMonths(timePeriod)
  return months.map((month) => ({
    month: new Date(month + "-01").toLocaleDateString("en-GB", { month: "short", year: "numeric" }),
    netWorth: monthlyData[month].reduce((sum, entry) => sum + entry.endingBalance, 0),
  }))
}

export function getNetWorthByAccountOverTime(timePeriod: TimePeriod = "all"): Array<any> {
  const months = getFilteredMonths(timePeriod)
  return months.map((month) => {
    const monthData: any = {
      month: new Date(month + "-01").toLocaleDateString("en-GB", { month: "short", year: "numeric" }),
...
export function getGrowthBySourceOverTime(timePeriod: TimePeriod = "all"): Array<any> {
  const months = getFilteredMonths(timePeriod)
...
export function getMonthlyTotals(month: string) {
  ...
  return {
    ...
    totalInterestEarned,
    totalCapitalGains,
    totalGrowth: totalSavingsFromIncome + totalInterestEarned + totalCapitalGains,
  }
}
...
export function getValueAtPeriod(accountId: number, timePeriod: ValueTimePeriod): number {
  const months = Object.keys(monthlyData).sort()
  const currentDate = new Date()
  ...
  switch (timePeriod) {
    case "1M":
      ...
      break
    case "3M":
      targetDate = new Date(currentDate.getFullYear(), currentDate.getMonth() - 3, 1)
      break
    case "6M":
      targetDate = new Date(currentDate.getFullYear(), currentDate.getMonth() - 6, 1)
      break
    case "1Y":
    ...
    case "ALL":
      ...
      const firstEntry = monthlyData[firstMonth]?.find((e) => e.accountId === accountId)
  ...
  if (monthlyData[targetMonthKey]) {
    const entry = monthlyData[targetMonthKey].find((e) => e.accountId === accountId)
  ...
  const entry = monthlyData[closestMonth]?.find((e) => e.accountId === accountId)
  return entry ? entry.endingBalance : 0
}
