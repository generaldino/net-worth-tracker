...
import {
  getNetWorthOverTime,
  getNetWorthByAccountOverTime,
  getGrowthBySourceOverTime,
  getMonthlyTotals,
  accounts,
  type TimePeriod,
  monthlyData, // Declare the variable here
} from "@/lib/data"
...
export function ChartSection() {
  ...
  const netWorthData = getNetWorthOverTime(timePeriod)
  const accountData = getNetWorthByAccountOverTime(timePeriod)
  const sourceData = getGrowthBySourceOverTime(timePeriod)
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-background border border-border rounded-lg p-3 shadow-lg">
          ...
          <p className="text-xs text-muted-foreground mt-2">Click to pin details</p>
        </div>
      )
    }
    return null
  ...
  const renderChart = () => {
    switch (chartType) {
      case "total":
        const totalBarSize = getBarSize(netWorthData.length)
        return (
          <ChartContainer
            config={{
              netWorth: {
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={netWorthData} margin={margins} barCategoryGap="15%">
      ...
      case "accounts":
        const accountBarSize = getBarSize(accountData.length)
        return (
          <ChartContainer
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={accountData} margin={margins} barCategoryGap="15%">
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis
                  dataKey="month"
                  fontSize={fontSize}
      ...
      case "sources":
        const sourceKeys = ["Savings from Income", "Interest Earned", "Capital Gains"]
        const sourceBarSize = getBarSize(sourceData.length)
        return (
          <ChartContainer
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={sourceData} margin={margins} barCategoryGap="15%">
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis
                  dataKey="month"
                  fontSize={fontSize}
  ...
  const renderDataDetails = (data: ClickedData) => {
    ...
    // Get the month key for detailed calculations
    const monthKey = Object.keys(monthlyData).find(
      (m) => new Date(m + "-01").toLocaleDateString("en-GB", { month: "short", year: "numeric" }) === month,
    )

    const monthlyTotals = monthKey ? getMonthlyTotals(monthKey) : null
    ...
    return (
      <div className="mt-4 p-4 bg-muted/30 rounded-lg border">
        ...
        {type === "total" && (
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-medium">£{chartData.netWorth.toLocaleString()}</span>
            </div>
            {monthlyTotals && (
              <>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Cash In:</span>
                  <span className="font-medium text-green-600">+£{monthlyTotals.totalCashIn.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Cash Out:</span>
                  <span className="font-medium text-red-600">-£{monthlyTotals.totalCashOut.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Net Cash Flow:</span>
                  <span
                    className={`font-medium ${monthlyTotals.totalCashFlow >= 0 ? "text-green-600" : "text-red-600"}`}
                  >
                    {monthlyTotals.totalCashFlow >= 0 ? "+" : ""}£{monthlyTotals.totalCashFlow.toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Account Growth:</span>
                  <span className={`font-medium ${monthlyTotals.totalGrowth >= 0 ? "text-green-600" : "text-red-600"}`}>
                    {monthlyTotals.totalGrowth >= 0 ? "+" : ""}£{monthlyTotals.totalGrowth.toLocaleString()}
                  </span>
                </div>
              </>
            )}
        ...
        {type === "accounts" && (
          <div className="space-y-2">
            <div className="text-sm text-muted-foreground mb-2">Account Breakdown:</div>
            {accounts.map((account, index) => {
              const value = chartData[account.name]
              if (value > 0) {
                return (
            ...
            <div className="border-t pt-2 mt-2">
              <div className="flex justify-between font-medium">
                <span>Total:</span>
                <span>
                  £{accounts.reduce((sum, account) => sum + (chartData[account.name] || 0), 0).toLocaleString()}
                </span>
              </div>
            </div>
          </div>
        ...
        {type === "sources" && (
          <div className="space-y-2">
            <div className="text-sm text-muted-foreground mb-2">Growth Sources:</div>
            {["Savings from Income", "Interest Earned", "Capital Gains"].map((source, index) => {
            ...
            <div className="border-t pt-2 mt-2">
              <div className="flex justify-between font-medium">
                ...
                <span
                  className={`${(chartData["Savings from Income"] + chartData["Interest Earned"] + chartData["Capital Gains"]) >= 0 ? "text-green-600" : "text-red-600"}`}
                >
                  {chartData["Savings from Income"] + chartData["Interest Earned"] + chartData["Capital Gains"] >= 0
                    ? "+"
                    : ""}
                  £
                  {(
    ...
    <Card>
      <CardHeader className="pb-3 sm:pb-6">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
          <div>
            <CardTitle className="text-lg sm:text-xl">Net Worth Analysis</CardTitle>
